...
import {PublicWritableSource, WritableSource, type SourceBinding} from './writable-source';
...
class TestSource extends WritableSource<number> {
  constructor() {
    super(signal(0));
  }
  ...
  doUpdate(fn: (v: number) => number) {
    this.update(fn);
  }

  doBind(fn: SourceBinding<number>) {
    return this.bindTo(fn);
  }
}
...
class TestPublicSource extends PublicWritableSource<string> {
  constructor() {
...
describe('Source', () => {
  describe('callable as function', () => {
    ...
    it('should be an instance of Source', async () => {
      ...
      expect(source).toBeInstanceOf(WritableSource);
    });
  });
  ...
  describe('bindSource', () => {
    it('should reactively bind from an external signal', async () => {
      ...
      runInInjectionContext(envInjector, () => source.doBind(() => external()));
    ...
    it('should not update when binding returns undefined', async () => {
      ...
      runInInjectionContext(envInjector, () =>
        source.doBind(() => (external() > 0 ? external() : undefined)),
      );
      fixture.detectChanges();
